...
export async function GET() {
  try {
    const res = await queryDb('SELECT * FROM websites ORDER BY id DESC').catch(() => ({ rows: [] }));
    return NextResponse.json({ success: true, table: 'websites', records: res.rows });
  } catch (error) {
...
export async function POST(request) {
  try {
    ...
    if (action === 'delete_record' || action === 'delete') {
      await queryDb('DELETE FROM websites WHERE id = $1', [body.id]);
    ...
    if (action === 'update_record' || action === 'update') {
      ...
      const res = await queryDb(
        `UPDATE websites SET ${setClauses.join(', ')} WHERE id = $${values.length} RETURNING *`,
        values
      );
