...
export async function GET() {
  try {
    const res = await queryDb('SELECT * FROM packages ORDER BY id ASC').catch(() => ({ rows: [] }));
...
export async function POST(request) {
  try {
    ...
    if (action === 'delete_record' || action === 'delete') {
      await queryDb('DELETE FROM packages WHERE id = $1', [body.id]);
    ...
    if (action === 'update_record' || action === 'update') {
      ...
      const res = await queryDb(
        `UPDATE packages SET ${setClauses.join(', ')} WHERE id = $${values.length} RETURNING *`,
        values
      );
    ...
    const res = await queryDb(
      `INSERT INTO packages (${keys.map((k) => `"${k}"`).join(', ')}) VALUES (${placeholders.join(', ')}) RETURNING *`,
      values
    );
    return NextResponse.json({ success: true, record: res.rows[0] });
  } catch (error) {
...
export async function DELETE(request) {
  try {
    ...
    await queryDb('DELETE FROM packages WHERE id = $1', [id]);
    return NextResponse.json({ success: true });
  } catch (error) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
