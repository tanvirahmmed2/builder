...
import { authenticateAdmin, clearAdminSessionCookie, hashPassword, isAdmin } from '@/lib/middleware/developer';
...
async function checkLastActiveAdminGuard(targetId, willDeactivateOrDelete = true) {
  ...
  if (role === 'admin' && admin.is_active) {
    const countRes = await queryDb("SELECT COUNT(*) as count FROM developers WHERE LOWER(role) = 'admin' AND is_active = TRUE");
...
export async function GET(request) {
  try {
    ...
    const [
      ...
      queryDb('SELECT id, name, email, role, is_active, is_verified, last_login_at, created_at FROM developers ORDER BY id ASC').then((r) => r.rows).catch(() => []),
  ...
  try {
    ...
    if (action === 'create_admin' || action === 'add_admin') {
      ...
      if (!authCheck.success) {
        return NextResponse.json(
          { success: false, error: authCheck.message || 'Forbidden: Only admin roles can create admin accounts.' },
    ...
    if (action === 'resend_verification_code' || action === 'resend_code') {
      ...
      const adminRes = await queryDb(
        `SELECT id, name, is_verified FROM developers WHERE LOWER(email) = $1 LIMIT 1`,
        [email]
      );
    ...
    if (action === 'remove_admin') {
      ...
      const guard = await checkLastActiveAdminGuard(body.adminId, true);
      if (guard) {
        return NextResponse.json({ success: false, error: guard.error }, { status: guard.status });
      }
      const res = await queryDb('DELETE FROM developers WHERE id = $1 RETURNING id', [body.adminId]);
    ...
    if (action === 'toggle_admin_status') {
      ...
      if (!authCheck.success) {
        return NextResponse.json(
          { success: false, error: authCheck.message || 'Forbidden: Only admin roles can update admin account status.' },
          { status: authCheck.status || 403 }
        );
      }
      const guard = await checkLastActiveAdminGuard(body.adminId, true);
      ...
      const res = await queryDb(
        'UPDATE developers SET is_active = NOT is_active WHERE id = $1 RETURNING id, name, email, is_active',
        [body.adminId]
      );
    ...
    if (action === 'recover_admin' || action === 'recover') {
      ...
      const dbAdminRes = await queryDb('SELECT * FROM developers WHERE LOWER(email) = $1 LIMIT 1', [email]);
