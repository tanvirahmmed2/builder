      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-slate-200 rounded-2xl p-6 shadow-xs">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Platform Features</h1>
          ...
          <p className="text-xs text-slate-500">Catalog of modular builder capabilities that can be bundled into packages.</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span>{showForm ? 'Hide Form' : 'Add Feature'}</span>
          </button>
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/80 text-slate-500 font-semibold uppercase tracking-wider text-[10px]">
                ...
                <th className="px-4 py-3 whitespace-nowrap">Feature Name</th>
                <th className="px-4 py-3 whitespace-nowrap">Key Identifier</th>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {loading ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-400">Loading features...</td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-slate-400">No features found.</td>
                </tr>
              ) : (
                filtered.map((feat) => (
                  <tr key={feat.id} className="hover:bg-slate-50/60 transition-colors">
